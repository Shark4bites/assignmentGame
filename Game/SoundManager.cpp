...
void SoundManager::playSound(string name){
	...
	for (soundListing listing : sounds){
		//if the listing matches the name we're searching, then play it :D
		if (listing.name == name){
			//params: which channel to play on(-1 is whichever available), the sound to play, how many times to loop (0 = no loop for you!)
			Mix_PlayChannel(-1, listing.sound, 0);
			break;//exit the loop so we dont waste any more time looking
		}
	}
}
